...
class _TeacherShareInvitePageState extends State<TeacherShareInvitePage> {
  ...
  Future<void> _loadInviteCode() async {
    ...
    try {
      ...
      if (response == null) {
        throw HttpException(404, 'Invite route not available.');
      }
      ...
      setState(() {
        _errorMessage = err.statusCode == 404 || err.statusCode == 405
            ? 'Invite route not available on server.'
            : err.message;
      });
    } catch (_) {
      setState(() {
  ...
  Future<void> _shareWhatsApp() async {
    ...
    await _trackInviteAction('share');
  ...
  Future<void> _trackInviteAction(String action) async {
    if (_inviteCode == '...') {
      return;
    }
    try {
      await _apiClient.postJson(
        '/api/v1/invites/my/track',
        token: HiveService.authToken,
        body: {'action': action},
      );
    } catch (_) {
      // Tracking is optional; ignore failures.
    }
  }
