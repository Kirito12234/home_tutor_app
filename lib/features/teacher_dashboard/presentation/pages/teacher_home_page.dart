...
class _TeacherHomePageState extends State<TeacherHomePage> {
    ...
    try {
      ...
      if (data is List) {
        final mapped = data.map<_CourseInfo>((item) {
          ...
          final rawDescription = course['description']?.toString().trim() ?? '';
          final description = rawDescription.isEmpty
              ? (course['category']?.toString().trim().isNotEmpty == true
                  ? course['category'].toString().trim()
                  : 'No description provided.')
              : rawDescription;
          final tutor = course['tutor'];
          final mentorName =
              course['instructorName']?.toString().trim().isNotEmpty == true
                  ? course['instructorName'].toString().trim()
                  : (tutor is Map<String, dynamic> &&
                          tutor['name']?.toString().trim().isNotEmpty == true
                      ? tutor['name'].toString().trim()
                      : 'Instructor');
          final studentsCount = (course['studentsCount'] as num?)?.toInt() ??
              (course['enrolledStudents'] as num?)?.toInt() ??
              (course['totalStudents'] as num?)?.toInt() ??
              0;
          final resolvedLessonCount =
              (course['lessonCount'] as num?)?.toInt() ??
                  (course['lessons'] is List
          ...
          final statusLabel = course['isNew'] == true
              ? 'New'
  ...
  String _extractCourseImageUrl(Map<String, dynamic> course) {
    ...
    final candidates = <String>[
      fromDynamic(course['imageUrl']),
      fromDynamic(course['image']),
      fromDynamic(course['courseImage']),
          ...
          children: [
            ClipRRect(
              ...
              child: resolvedImageUrl.isEmpty
                  ...
                  : isLocal
                      ? Image.file(
                          File(resolvedImageUrl.replaceFirst('file://', '')),
                          height: 62,
                          width: double.infinity,
                          fit: BoxFit.cover,
